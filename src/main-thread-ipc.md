...
| `KeyboardEvents.updateEscapeAbsorptionTap` — `CGEvent.tapEnable` | WindowServer event server | dismissal | the two calls measured separately: the `tapIsEnabled` READ was 4–34ms and once 246ms, the `tapEnable` write 1–7ms. Lumped together they read as "0–8ms" and the expensive half stayed hidden | moved from `beginHideUi` to `endHideUi`; the read is gone — the wanted state is compared against a cached flag, and only `.tapDisabledBy*` / wake / unlock force a write |
